...
public class bookFunctions {
	...
	public void readDataFromAuthorID() throws IOException {
		...
		BufferedReader br = new BufferedReader(
				new FileReader("C:\\Users\\rasim-pc\\git\\rovingyDB\\rovingyDB\\src\\rovingyDB\\id.txt"));
	...
	private String getAuthorImageURL(String authorContent) {
		...
		int index = authorContent.indexOf("http://www.neokur.com/photos/unlu/");

		String url = authorContent.substring(index, authorContent.indexOf("\" />", index));
	...
	private String getAuthorName(String authorContent) {
		...
		int index = authorContent.indexOf("f_28 w_b line_h7 p_m_0\">");

		String url = authorContent.substring(index + "f_28 w_b line_h7 p_m_0\">".length(),
				authorContent.indexOf("</h1>", index));
	...
	private String getAuthorBooksURL(String authorContent) {
		...
		int index = authorContent.indexOf("og:url\" content=\"");

		String url = authorContent.substring(index + "og:url\" content=\"".length(),
				authorContent.indexOf("\" />", index));
	...
	private String getBooksURL(String authorBooksContent) {
		...
		int index = authorBooksContent.indexOf("p_10 border_b_solid_pad0");

		if (index == -1)
			return null;

		int beginningIndex = authorBooksContent.indexOf("http://www.neokur.com/kitap", index);

		String url = authorBooksContent.substring(beginningIndex,
				authorBooksContent.indexOf("\" class", beginningIndex));
	...
	private String getBookID(String bookURL) {
		...
		String ID = bookURL.substring(bookURL.indexOf(bookIdentifier) + bookIdentifier.length(),
				bookURL.lastIndexOf("/"));
	...
	private String getBookImageURL(String authorBooksContent) {
		...
		int beginningIndex = authorBooksContent.indexOf("background-image:url(", index)
				+ "background-image:url(".length();

		String url = authorBooksContent.substring(beginningIndex,
				authorBooksContent.indexOf(")\"></div>", beginningIndex));
	...
	private String getBookName(String authorBooksContent) {
		...
		int beginningIndex = authorBooksContent.indexOf("c_mavi_koyu w_b\" >", index) + "c_mavi_koyu w_b\" >".length();

		String url = authorBooksContent.substring(beginningIndex, authorBooksContent.indexOf("</a>", beginningIndex));
	...
	private String getPublishedYear(String bookContent) {
		...
		if (bookContent.indexOf(bookIdentifier) > -1) {
			...
			year = bookContent.substring(bookContent.indexOf(bookIdentifier) + bookIdentifier.length(),
					bookContent.indexOf("Neokur Puaný"));

			year = year.replace("</td>", "").replace("<td valign=\"top\">", "").replace("<tr>", "").replace("</tr>", "")
					.replace(":", "").trim();
		}
	...
	private String getQuote(String quotePageContent) {
		...
		int index = quotePageContent.indexOf("<div class=\"f_14\" >");

		if (index == -1)
			return null;

		int beginningIndex = quotePageContent.indexOf("<div class=\"f_14\" >", index)
				+ "<div class=\"f_14\" >".length();

		String quote = quotePageContent.substring(beginningIndex, quotePageContent.indexOf("</div>", beginningIndex));

		quote = quote.replace("<br />", "\n").trim();

		quote = clearTags(quote);

		for (int i = 0; i < quote.length() - 1; i++) {
			if (Character.isUpperCase(quote.charAt(i + 1)) && Character.isLowerCase(quote.charAt(i))) {
				quote = quote.substring(0, i + 1) + "\n" + quote.substring(i + 1);
			}
		}
	...
	public void getDataFromAuthorID(String authorID) throws Exception {
		...
		while (true) {
			...
			while (true) {
				...
				if (bookURL == null) {
					...
					while (true) {
						...
						while (true) {
							...
							quotePageContent = quotePageContent
									.substring(quotePageContent.indexOf("<div class=\"f_14\" >")
											+ "<div class=\"f_14\" >".length());
						}
				...
				authorBooksContent = authorBooksContent.substring(
						authorBooksContent.indexOf("p_10 border_b_solid_pad0") + "p_10 border_b_solid_pad0".length());
			}
			index++;
		}
	}

	public void insertToBookTable(String bookID, String name, String AuthorID) {
		...
		if (!isAdded(bookID, "Books")) {
			...
			dbMobile.executeUpdate("insert into Books (ID,Name,AuthorID,LikeCount,CreDate) values (" + bookID + ",'"
					+ name.replace("'", "''") + "'," + AuthorID + ",0,'"+ datetime +"')");
		}
	}
	...
	public void insertToAuthorTable(String name, String AuthorID) {
		if (!isAdded(AuthorID, "Author")) {
			dbMobile.executeUpdate("insert into Author (ID,Name) values (" + AuthorID + ",'" + name + "')");
		}
	...
	public void insertDummyLike() throws SQLException {
		...
		while (rs.next()) {
			...
			for (int i = 0; i < likeCount; i++) {
				...
				dbMobile.executeUpdate("insert into Device_Like (DateTime,DeviceID,Type,ObjectID) values (NOW(),'Dummy"
						+ i + "',2," + id + ")");
			}
		}
	}
	...
	public void filterQuotes() throws SQLException {
		...
		while (rsBook.next()) {
			...
			ResultSet rsQuote = dbMobile.executeQuery("SELECT ID,BookID,Text,LikeCount FROM newQuotes where BookID="
					+ id + " order by LikeCount Desc limit 0,20");
	...
	public void clearDublicateBooks() {
		dbMobile.connectToDB(dbType.KITAPP);
		ResultSet rs = dbMobile.executeQuery(
				"SELECT `AuthorID`,`Name`,count(*) FROM `Books` group by `AuthorID`,`Name`  having count(*)>1");
		...
		try {
			while (rs.next()) {
				...
				ResultSet rs2 = dbMobile.executeQuery("SELECT * FROM `Books` where Name='" + name.replace("'", "''")
				...
				while (rs2.next()) {
					...
					if (isFirst) {
						...
						dbMobile.executeUpdate(
								"Update Device_Like set ObjectID=" + bookID + " where Type=1 and ObjectID=" + bID);
					}
				}
				dbMobile.executeUpdate(
						"Update Books set LikeCount=(Select count(*) from Device_Like where Type=1 and ObjectID="
								+ bookID + ") where ID=" + bookID);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
